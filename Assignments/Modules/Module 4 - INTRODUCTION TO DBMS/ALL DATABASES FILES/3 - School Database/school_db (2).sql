...
--
-- Host: 127.0.0.1:3307
-- Generation Time: Mar 17, 2026 at 12:12 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `display_employee_names` ()   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `show_courses` ()   BEGIN 
        ...
        DECLARE course_cursor CURSOR FOR SELECT course_name, course_duration FROM courses;
        ...
        course_loop: LOOP
        	...
        	FETCH course_cursor INTO cname, duration;
            
            IF done = 1 THEN 
            	LEAVE course_loop;
            END IF;
            
            SELECT cname AS Course_name, duration AS Duration;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `show_employee_details` ()   BEGIN
    	...
    	DECLARE emp_cursor CURSOR FOR SELECT emp_id, emp_name, dept_id FROM employees;
        ...
        read_loop: LOOP 
            ...
            SELECT eid AS ID, ename AS Name, dept AS Department;
